...
- If USDPORT-Powered on was entered from any state except USDPORT.Disabled then this transition shall take place if Far-end Receiver Terminations (R_RX-DC) are not detected after eight successive Rx.Detect.Quiet to Rx.Detect.Active transitions.
...
- From the USDPort.Training state if a timeout occurs on any Polling substate (see Figure 7-21).
